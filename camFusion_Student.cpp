...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    //get the matches within curr_boundingBox
    for(auto it=kptMatches.begin(); it != kptMatches.end(); ++ it)
    {
        ...
        // check wether point is within current bounding box
        if (smallerBox_c.contains(train_pt)) 
                     ...
                     std::vector<LidarPoint> &lidarPointsCurr, double frameRate, double &TTC)
{
    ...
    mean_px = mean_px /p_size;
    mean_cx = mean_cx /c_size;
    std::cout<<"lidar_mean_px:"<<mean_px<<std::endl;
    std::cout<<"lidar_mean_cx:"<<mean_cx<<std::endl;

    float dt = 1/frameRate;
    TTC = mean_cx * dt / (mean_px - mean_cx);
}
